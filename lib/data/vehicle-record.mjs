...
/**
 * Build the display row for a job, resolving vehicle and customer through
 * their relations so nothing is read from a duplicated string.
 *
 * @param {import("./repository.d.mts").Repository} repository
 * @param {import("./schema.d.mts").Job} job
 */
export function buildJobRow(repository, job) {
  const vehicle = repository.getVehicle(job.vehicle_id);
  const customer = vehicle?.customer_id ? repository.getCustomer(vehicle.customer_id) : null;
  return { job, vehicle: vehicle ?? null, customer };
}

/**
 * Filter and categorise open jobs for the Work screen.
 *
 * scope "today"   — everything not done/cancelled (same view as "active" for now,
 *                   which matches the original behaviour of showing all open work)
 * scope "active"  — in_progress only
 * scope "history" — done and cancelled
 */
export function filterJobRows(rows, scope) {
  if (scope === "history") return rows.filter((r) => r.job.status === "done" || r.job.status === "cancelled");
  if (scope === "active")  return rows.filter((r) => r.job.status === "in_progress");
  // "today" = all non-completed: scheduled + in_progress
  return rows.filter((r) => r.job.status === "scheduled" || r.job.status === "in_progress");
}
...
/**
 * Build the display row for a customer, resolving their vehicles through
 * customer_id. Two customers with the same name each get their own row
 * with their own vehicle list.
 *
 * @param {import("./repository.d.mts").Repository} repository
 * @param {import("./schema.d.mts").Customer} customer
 */
export function buildCustomerRow(repository, customer) {
  const vehicles = repository.listVehiclesByCustomer(customer.id);
  return { customer, vehicles, vehicleCount: vehicles.length };
}

/**
 * Case-insensitive match on name or phone. Never touches vehicle strings
 * or any other table — the search is on the customer record only.
 */
export function matchesCustomerQuery(row, query) {
  const needle = String(query ?? "").trim().toLowerCase();
  if (!needle) return true;
  return (
    row.customer.name.toLowerCase().includes(needle) ||
    (row.customer.phone ?? "").replace(/\s/g, "").includes(needle.replace(/\s/g, ""))
  );
}
...
/**
 * Live counts for the dashboard metrics and the recent-activity list.
 * Replaces the three hardcoded numbers and the hardcoded jobs array.
 */
export function buildDashboardSummary(repository) {
  const openJobs = repository.listOpenJobs();
  const openCount = openJobs.length;

  // Build vehicle and customer maps once — one query each, no N+1.
  const vehicleMap = new Map(repository.listVehicles().map((v) => [v.id, v]));
  const customerMap = new Map(repository.listCustomers().map((c) => [c.id, c]));

  // Recent: last 3 open jobs, each resolved to a JobRow so the vehicle is
  // already available and the render needs no further lookups.
  const recent = openJobs.slice(0, 3).map((job) => {
    const vehicle = vehicleMap.get(job.vehicle_id) ?? null;
    const customer = vehicle?.customer_id ? (customerMap.get(vehicle.customer_id) ?? null) : null;
    return { job, vehicle, customer };
  });

  // Note count: one pass over the vehicle map, not listVehicles() again.
  let noteCount = 0;
  for (const vehicle of vehicleMap.values()) {
    noteCount += repository.listNotesByVehicle(vehicle.id).length;
  }

  return { openCount, noteCount, recent };
}
