 ...
 * @param {string} [options.garageName]
 ...
 * @param {boolean} [options.seedGarageWhenEmpty]
 * @param {(data: object) => void} [options.onPersist]
 */
export function createRepository({
  garageId = DEMO_GARAGE_ID,
  garageName = "Motofy Garage",
  storage = createMemoryStorage(),
  now = () => new Date(),
  generateId,
  seedWhenEmpty = true,
  seedGarageWhenEmpty = false,
  onPersist,
} = {}) {
  ...
  function persist() {
    try {
      ...
      if (onPersist) onPersist(clone(data));
    } catch {
  ...
  // A shared browser dataset can already contain another garage while the
  // dedicated demo garage is missing. Add only this garage's sample records
  // instead of replacing the whole dataset and losing real user data.
  const garageHasRecords = ["customers", "vehicles", "jobs", "notes"]
    .some((table) => data[table].some((row) => row.garage_id === garageId));
  if (seedGarageWhenEmpty && !garageHasRecords) {
    const seed = createSeed({ now: now(), garageId });
    for (const table of ["customers", "vehicles", "jobs", "notes"]) {
      data[table].push(...seed[table]);
    }
    if (!data.garages.some((row) => row.id === garageId)) data.garages.push(...seed.garages);
    loadStatus = "seeded_garage";
    persist();
  }

  // A new login identity gets an empty, independent garage inside the same
  // dataset. Existing garages and their records remain untouched.
  if (!data.garages.some((row) => row.id === garageId)) {
    data.garages.push(FACTORIES.garages({ id: garageId, name: garageName }, stamp()));
    persist();
  }
