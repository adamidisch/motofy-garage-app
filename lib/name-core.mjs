 ...
 * sees the key. Gemini decides first, with a small deterministic safety net
 * for common Greeklish names when the upstream call is unavailable.
 */
...
export const NAME_MODEL = "gemini-3.5-flash-lite";
...
export const NAME_PROMPT =
  ...
  "Examples: Giannos -> Γιάννο, Giannis -> Γιάννη, Rolandos -> Ρολάνδε, Andreas -> Αντρέα, Marios -> Μάριε, Panikos -> Πανίκο. " +
...
export function cleanGreeting(raw, fallback) {
  ...
  const greeklishFixes = {
    panikos: "Πανίκο",
  };
  const fallbackSafe = greeklishFixes[fallbackName.toLocaleLowerCase("en-US")] ?? fallbackName;
  if (raw === null || raw === undefined) return fallbackSafe;
  let text = String(raw).trim().replace(/^["'«»]+|["'«»]+$/g, "");
  text = text.replace(/^(καλημέρα|καλησπέρα|good morning|good evening|hello|hi)[,:\s]+/i, "");
  text = text.split(/[\n,]/)[0].trim();
  if (!text || text.length > 40) return fallbackSafe;
  const vocativeFixes = {
    "Γιάννος": "Γιάννο",
    "Γιάννης": "Γιάννη",
    "Ρολάνδος": "Ρολάνδε",
  ...
  return greeklishFixes[text.toLocaleLowerCase("en-US")] ?? vocativeFixes[text] ?? text;
}
...
/** @param {{ apiKey?: string; name?: string; lang?: string; model?: string; fetchImpl?: typeof fetch; log?: (message: string, ...rest: unknown[]) => void }} options */
...
export async function requestNormalizedName(name, lang = "el", fetchImpl = fetch) {
  ...
  try {
    ...
    if (!response.ok) return cleanGreeting(typed, typed);
    const data = await response.json();
    return cleanGreeting(data?.greeting, typed);
  } catch {
    return cleanGreeting(typed, typed);
  }
}
