...
import { Camera, Check, ChevronRight, CircleDashed, Clock3, Gauge, ImageIcon, Mail, Package, Phone, Plus, Share2, StickyNote, Trash2, UserRound, Wrench, X } from "lucide-react";
import styles from "./motofy2.module.css";
import { pickPhoto } from "../lib/data/photo-store.mjs";
import { addCost, addPart, cyclePart, removeCost, removePart, setCustomerNote, setReady, totalCost } from "../lib/data/workflow-store.mjs";
...
export function PartsScreen({ jobId, workflow, lang, refresh }: { jobId: string; workflow: JobWorkflow; lang: "el" | "en"; refresh: () => void }) {
  ...
  return <div className={styles.workflowBody}>
    <section className={styles.workflowCard}><p className={styles.sectionLabel}>{greek ? "ΝΕΟ ΑΝΤΑΛΛΑΚΤΙΚΟ" : "ADD PART"}</p><div className={styles.inlineEntry}><input value={label} onChange={(e) => setLabel(e.target.value)} placeholder={greek ? "π.χ. τακάκια εμπρός" : "e.g. front brake pads"}/><button aria-label={greek ? "Προσθήκη ανταλλακτικού" : "Add part"} onClick={() => { if (!label.trim()) return; addPart(jobId, label); setLabel(""); refresh(); }}><Plus size={18}/></button></div></section>
    <section className={styles.partsList}>{workflow.parts.length ? workflow.parts.map((part: WorkflowPart) => <article key={part.id}><div><strong>{part.label}</strong><button className={styles.partStatus} aria-label={`${greek ? "Αλλαγή κατάστασης για" : "Change status for"} ${part.label}`} onClick={() => { cyclePart(jobId, part.id); refresh(); }}>{labels[part.status] ?? part.status}</button></div><button aria-label={`${greek ? "Διαγραφή ανταλλακτικού" : "Remove part"}: ${part.label}`} onClick={() => { removePart(jobId, part.id); refresh(); }}><Trash2 size={15}/></button></article>) : <EmptyPanel icon={<Package size={20}/>} title={greek ? "Δεν υπάρχουν ανταλλακτικά" : "No parts yet"}/>}</section>
  </div>;
}

export function CostScreen({ jobId, workflow, lang, refresh }: { jobId: string; workflow: JobWorkflow; lang: "el" | "en"; refresh: () => void }) {
  ...
  return <div className={styles.workflowBody}>
    ...
    <section className={styles.workflowCard}><div className={styles.costEntry}><input value={label} onChange={(e) => setLabel(e.target.value)} placeholder={greek ? "Εργασία / ανταλλακτικό" : "Work / part"}/><input value={amount} onChange={(e) => setAmount(e.target.value)} inputMode="decimal" placeholder="€"/><button aria-label={greek ? "Προσθήκη κόστους" : "Add cost"} onClick={() => { addCost(jobId, label, amount); setLabel(""); setAmount(""); refresh(); }}><Plus size={18}/></button></div></section>
    <section className={styles.costList}>{workflow.costs.map((row: WorkflowCost) => <article key={row.id}><span>{row.label}</span><strong>€{Number(row.amount).toFixed(2)}</strong><button aria-label={`${greek ? "Διαγραφή κόστους" : "Remove cost"}: ${row.label}`} onClick={() => { removeCost(jobId, row.id); refresh(); }}><X size={14}/></button></article>)}</section>
  </div>;
}

export function CheckoutScreen({ job, workflow, record, lang, onComplete }: { job: Job; workflow: JobWorkflow; record: VehicleRecordModel; lang: "el" | "en"; onComplete: () => void }) {
...
export function MotofyMePreview({ record, job, workflow, lang }: { record: VehicleRecordModel; job: Job; workflow: JobWorkflow; lang: "el" | "en" }) {
...
export function EmptyPanel({ icon, title, hint }: { icon: React.ReactNode; title: string; hint?: string }) { return <div className={styles.empty}><span>{icon}</span><strong>{title}</strong>{hint && <small>{hint}</small>}</div>; }
export function statusLabel(status: string, t: Copy) { if (status === "in_progress") return t.statusInProgress; if (status === "done") return t.statusDone; if (status === "cancelled") return t.statusCancelled; return t.statusScheduled; }

export function Overview({ t, lang, vehicle, customer, currentJob, lastActivity, scanSuggestion, empty, openCustomerTab, openCreation }: { t: Copy; lang: "el" | "en"; vehicle: Vehicle; customer: VehicleRecordModel["customer"]; currentJob: Job | null; lastActivity: VehicleRecordModel["lastActivity"]; scanSuggestion: VehicleRecordModel["scanSuggestion"]; empty: VehicleRecordModel["empty"]; openCustomerTab: () => void; openCreation: (mode: string) => void }) {
  return <>
...
export function Notes({ t, lang, notes, empty }: { t: Copy; lang: "el" | "en"; notes: VehicleRecordModel["notes"]; empty: VehicleRecordModel["empty"] }) {
